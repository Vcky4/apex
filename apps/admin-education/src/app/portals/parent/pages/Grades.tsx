...
export default function Grades() {
  ...
  const courseGrades: CourseGrade[] = [
    ...
    },
    {
      ...
      categoryBreakdown: [
        { category: 'Quizzes', weight: 25, average: 96 },
        { category: 'Assignments', weight: 30, average: 94 },
        { category: 'Midterm', weight: 20, average: 0 },
        { category: 'Final Exam', weight: 25, average: 0 }
      ]
    },
    {
      ...
      gradeItems: [
        {
          ...
          courseName: 'English Language Arts',
          assignmentType: 'Essay',
          assignmentName: 'Creative Writing Assignment',
          date: '2024-02-05',
          score: 98,
          maxScore: 100,
          percentage: 98,
      ...
      categoryBreakdown: [
        { category: 'Essays', weight: 35, average: 98 },
        { category: 'Quizzes', weight: 25, average: 95 },
        { category: 'Final Exam', weight: 40, average: 0 }
      ]
    }
  ];

  const filteredGrades = courseGrades.filter(cg => {
    if (selectedStudent !== 'All' && cg.studentName !== selectedStudent) return false;
    if (selectedCourse !== 'All' && cg.courseCode !== selectedCourse) return false;
    return true;
  });

  const allGradeItems = filteredGrades.flatMap(cg => cg.gradeItems);
  const students = Array.from(new Set(courseGrades.map(cg => cg.studentName)));
  const courses = Array.from(new Set(courseGrades.map(cg => cg.courseCode)));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-charcoal-gray">Grades & Progress</h1>
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-gray-500">({course.studentName})</span>
                  </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {recentGrades.map((item) => (
                  <tr key={item.id}>
                    <td className="px-4 py-3 text-sm font-medium text-charcoal-gray">{item.studentName}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{item.courseCode}</td>
